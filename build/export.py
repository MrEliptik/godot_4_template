...
gh_cli_path = 'GITHUB_CLI_PATH'
butler_path = 'BUTLER_PATH'
butler_game = 'ITCH_USERNAME/GAME_NAME'
...
web_template_itch = "HTML5 ITCH"

templates = [windows_template_itch, linux_template_itch, web_template_itch]

platforms_available = ['steam', "itch"]
args = {}
...
def parse_latest_from_folder(path):
    ...
    for folder in subfolders:
        if folder["name"] == "archive": continue
...
def export_template(template, build_path, build_number):
    ...
    print_console("    |---> Executing command: {}".format(cmd))

    with subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1, encoding='utf-8') as sp:
        for line in sp.stdout:
            print(line.strip())
            #if 'savepack: end' in line.strip():
                #print_console("END FOUND")
                # sp.terminate()

    print_console("    |---> Exporting template finished: {}".format(template))
...
def main():
    ...
    if args.upload_latest:
        latest_build_dict = parse_latest_from_folder(build_path)
    ...
    if args.force_upload:
        ...
        platforms = []
        # If we specified platform in arguments, use that, otherwise
        # default to platforms available
        if args.platform != '':
